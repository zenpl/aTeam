...
/**
 * t-200：紧凑记法也从同一处拿「这个数在现在的哪一侧」。
 *
 * 它不是第五把梯子，也不自己决定负数说什么——`band` 答 null，它就往上传，由调用方说。`fmt.board` 那一路的
 * 决定写在它自己身上（当 0，与页面那两处 Math.max(0, …) 同一个决定）。
 */
describe("t-200 · 紧凑记法不自己判负", () => {
  it("另一侧答 null，不掉进最小档", () => {
    expect(compact(-1)).toBeNull();
    expect(compact(-4 * 3600_000)).toBeNull();
    expect(compact(0)).toBe("0s");
  });
});
