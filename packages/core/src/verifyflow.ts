...
export function dueDefaults(s: State, now: Date): NewEvent[] {
  ...
  for (const st of s.instructions.values()) {
    ...
    // t-215 判据 6：**这张卡声明的条件已经被改动过，那就不许无声地按默认结掉。**
    // pd 那张 Q25 是真样本：默认那一支 A 的正文写「明早开工时」，而那个明早已经过去——按一个描述已失效的默认
    // 落一条「已执行」，等于替人执行一件做不到的事。所以这里**什么都不落**：卡留着等人答，牌桌上那一格
    // （stale_since）说得出它为什么被扣着。
    // **要给人看的那句话没写**（pd 的措辞冻结开着，判据 5）：此刻它只是数据，不是话。
    if (st.stale_since) continue;
