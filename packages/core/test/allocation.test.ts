...
import { MemoryStore, append, reduce, board, allocation, staticAllocation, runtimeAllocation, selfCorrections, classifyFollowUp, similarity, saidHops, type NewEvent } from "../src/index.js";
...
describe("runtime metrics over the last window", () => {
  it("低效: seams resolved by one role, a slow acker, a deep verify queue; quiet teams show nothing", async () => {
    ...
    ws = runtimeAllocation(await w.state(), w.at(), HUMAN);
    // twice its bar, against 1.04x for the seams and 1.0x for the queue: this is the one worth a person's attention
    expect(ws[0].evidence).toEqual(["dev 的 ack 延迟 p90 30 分钟，其他角色中位数 1 分钟"]);
    expect(ws[0].hint.startsWith("dev 的 ack 延迟 p90 30 分钟")).toBe(true);
    expect(ws.filter((x) => x.pattern === "低效")).toHaveLength(1); // one entry, three findings, one of them said
  });
  ...
  it("无效上下文移交: a forwarded instruction, and a sentence that took more than two hops to a task", async () => {
    ...
    expect(ws[0].evidence[0]).toBe("pm 十分钟内转发了 2 条别人的话（正文相似度 ≥ 0.6）");   // 2 forwards outranks a 3-hop sentence
  ...
  it("负载陷阱 (runtime): one recipient gets most instructions, and the loudest of the two findings is the one said", async () => {
    const w = world();
    const ackBy = () => new Date(w.at().getTime() + min(60)).toISOString();
    ...
    const ws = runtimeAllocation(await w.state(), w.at(), HUMAN);
    expect(ws).toEqual([{ pattern: "负载陷阱", evidence: ["pm 收到 73% 的指令（8/11）"], hint: "pm 收到 73% 的指令（8/11）。建议：把它持有的、能改成规则或服务的职责先拿走。" }]);
  });

  /**
   * t-123 判据 1: 「我上一条写错了」 and 「生产刚换了 sha，重发一次」 are opposite facts, and the first version of this
   * counted them as one number — every second instruction to the same person inside ten minutes, whatever it said.
   * On the real log of 22:24–00:35 that read 71.9%; pm's hand count of its own mistakes was 4 out of 96.
   */
  describe("t-123 · 自己写错的更正与情况变了的更新分开数", () => {
    it("classifies a follow-up by what it says, and counts what it cannot classify as neither", async () => {
      const w = world();
      const ackBy = () => new Date(w.at().getTime() + min(60)).toISOString();
      const say = (to: string, body: string, plus?: number) => w.emit({ kind: "instruction", actor: "pd", to, body, ack_by: ackBy() }, plus);
      // eight standing alone (twelve minutes apart: nothing follows anything), then four pairs a minute apart
      ...
      await say("pm", "生产已上线 abc1234，这条按新的来", min(1));      // the world moved: not a mistake
      await say("pm", "再来一件", min(12));
      await say("pm", "另外那件也麻烦你看一下", min(1));                 // says neither: nobody can call this
      // dev gets as many, so no one recipient is over half and the load line stays out of the way
    ...
    it("the four pm really wrote that night are corrections; the ones a looser pattern caught are not", () => {
      // verbatim from the log, 22:24–00:35 — the calibration window of 判据 3
      for (const body of [
        "更正：验 7125bde 而不是 b27980d（dev 的断言没跟上，7125bde 全绿，实现不变）。",
        "更正我 23:15 那条：「新工作另开分支」是我说错了，harness 把你钉在 claude/frontend-j8z8jj",
        "再更正一次我 23:15 那条：冻的是 sha，不是分支。be9b232 不会因为你继续提交而改变。",
        "更正我 23:25 那条：t-090 请按 65a6ef4 验，不是 61cd58a。",
      ]) expect(classifyFollowUp(body)).toBe("更正");
      // these all contain a correction word and none of them is somebody correcting their own instruction
      for (const body of [
        "自我更正率口径请拆成「情况变了」与「自己写错」两类",
        "②③的做法要你点头：t-064 撤回、t-073/t-074 合并",
        "t-103 判据已按 pd 23:52 的决策改写（我 23:51 那五条作废）",
        "t-079 判据1 以 pd 22:45 决策为准，我的「不留灰字」作废",
      ]) expect(classifyFollowUp(body)).not.toBe("更正");
      // the world moving is its own class, never counted with the mistakes
      expect(classifyFollowUp("生产已是 babae6d，你的修法还在它之后")).toBe("更新");
      expect(classifyFollowUp("请把 t-091 提交上去")).toBe("说不好");
      // taking an instruction back says plainly that it should not have been sent
      expect(classifyFollowUp("", true)).toBe("更正");
    });
  });
