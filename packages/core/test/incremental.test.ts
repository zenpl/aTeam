...
async function busyLog(store: MemoryStore): Promise<Event[]> {
  ...
  await put({ kind: "task", op: "create", actor: "pm", task: "t-a", title: "甲", criteria: ["能用"] , no_human_impact: true}, 9);
  await put({ kind: "task", op: "create", actor: "pm", task: "t-b", title: "乙", criteria: ["能用"] , no_human_impact: true}, 10);
  await put({ kind: "task", op: "create", actor: "pm", task: "t-c", title: "丙", criteria: ["能用"] , no_human_impact: true}, 11);
  ...
  await put({ kind: "task", op: "create", actor: "pm", task: "t-d", title: "丁", criteria: ["能用"] , no_human_impact: true}, 20);
