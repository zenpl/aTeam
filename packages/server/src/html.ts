  ...
  if (asks.length || reopen) {
    ...
    for (const i of asks) {
      ...
      if (isContactCard(i)) {
        ...
        const dueLine = say ? "" : `<span class="hint">${esc(UI.due(atClock(i.ack_by_again ?? i.ack_by)))}</span>`;
