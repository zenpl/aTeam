...
/**
 * **期限：超过它这次就不算 ok。** 这个数是怎么定的——用 qa 00:14 那一批实测，取两群之间那条空得最宽的缝：
 * 稳态九次落在 **0.22–0.28s**，卡住那段是 8.32／13.99／18.28／20.84／21.25／22.42／34.56／**48.13**s。
 * 两群之间从 0.28 到 8.32 隔着 30 倍，**1 秒在缝的中间**：比最慢的稳态宽 3.6 倍（不会把正常的一下抖动报成病），
 * 比最快的卡住紧 8.3 倍（最轻的一次卡也逃不掉）。**它不是拍的，是量出来的两群之间那条缝。**
 * 哪天稳态真的变慢到逼近 1 秒，改这个数要带一批新的实测，不许只改数字。
 */
export const HEALTH_BUDGET_MS = 1000;
